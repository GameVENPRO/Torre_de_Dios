...
def PROFILE_Kb():
    ...
    pfl_kb.add(*[KeyboardButton(name) for name in['🥋 Equipamiento','⚖️ Mejorar las características','📯 Ascenso de rango','🔙 Atrás']])
    return pfl_kb


def EQUIPMENT_Kb():
    reply_kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True , row_width=2)
    reply_kb.add(*[KeyboardButton(name) for name in['📤 Retirar el equipo','⚒ Kraft','🛒 Mercado','🔙 Atrás']])
    return reply_kb


def HEALING_Kb():
    reply_kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    reply_kb.add(*[KeyboardButton(name) for name in['💊 Enfermería','🧪 Pociones curativas','🔙 Atrás']])
    return reply_kb


def STATS_INC_Kb():
    kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    kb.add(*[KeyboardButton(name) for name in['⚖️ Mejorar las características','🔙 Atrás']])
    return kb
